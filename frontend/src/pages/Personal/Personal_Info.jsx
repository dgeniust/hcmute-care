...
import {notifyErrorWithCustomMessage, handleHttpStatusCode, notifySuccessWithCustomMessage} from '../../utils/notificationHelper';
...
const Personal_Info = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      notifyErrorWithCustomMessage('Lỗi khi tải dữ liệu từ localStorage', messageApi);
    }
  },[messageApi])
  ...
  const handleSubmitUpdate = async () => {
    ...
    const fullname = formData.lastName ? `${formData.lastName} ${formData.firstName}` : formData.firstName;
    ...
    try{
        const response = await fetch(`http://localhost:8080/api/v1/customers/${formData.id}`, {
            method: 'PUT',
            headers: {
        ...
        notifySuccessWithCustomMessage("Cập nhật thông tin thành công", messageApi)
        setTimeout(() => {
            window.location.reload();
        }, 1000);
    }
    catch(e) {
        notifyErrorWithCustomMessage('Lỗi kết nối khi cập nhật thông tin', messageApi);
                    ...
                    <div className='w-[150px] h-[150px] rounded-full bg-white absolute border border-black top-1/2 left-1/6 transform -translate-x-1/2 -translate-y-1/3'>
                        {
                            formData && formData.gender === "nam" ? (
                                ...
                                <img src="https://api.dicebear.com/9.x/miniavs/svg?seed=Liliana" alt="" srcset="" 
                                className="object-center w-full h-full rounded-full" />
                            )
                        }
