...
const MedicalRecord = () => {
    ...
    const handleDataMedicalRecord = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Error fetching data: ${errorText || response.statusText}`
          );
          handleHttpStatusCode(
            response.status,
            "",
  ...
  return (
    <div className="w-full h-full p-8 border border-black">
      <h1 className="text-black font-bold text-xl text-center w-full ">
        Hồ sơ người bệnh
      </h1>
      ...
      >
        <div className="text-center">
          <h1 className="text-lg mb-4 font-bold">CHỌN CHỨC NĂNG</h1>
        </div>
