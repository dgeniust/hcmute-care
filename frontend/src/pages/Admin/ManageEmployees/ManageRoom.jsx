...
import { Table, Button, Modal, Form, Input, InputNumber, message, Tag, Space, Typography, Tooltip } from 'antd';
...
export default function ManageRoom() {
  ...
  const fetchRooms = async (page = 1) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/v1/room-details?page=${page}&size=10&sort=id&direction=asc`);
  ...
  const handleDelete = async (id) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/v1/room-details/${id}`, { 
        method: 'DELETE' 
      });
  ...
  const handleOk = async () => {
    try {
      ...
      const url = editingRoom
        ? `http://localhost:8080/api/v1/room-details/${editingRoom.id}`
        : 'http://localhost:8080/api/v1/room-details';
    ...
    <div className="p-6 bg-gray-50 w-full min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-6">
          <Title level={3} className="m-0">Room Management</Title>
          <Button 
            type="primary" 
            icon={<PlusOutlined />} 
            onClick={handleAdd}
