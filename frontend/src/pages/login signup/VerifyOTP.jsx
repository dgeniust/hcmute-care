...
const Signup = () => {
  ...
  function resetOTP() {
    ...
    notifySuccess(
      ...
      "Gửi lại mã OTP thành công, vui lòng kiểm tra tin nhắn SMS của bạn để xác thực tài khoản 🥳",
      notificationApi
    );
  }
  const onChange = (text) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (type === "signup") {
      ...
      if (!isEmptyOTP) {
        ...
        const response = await fetch(`${apiUrl}v1/auth/register/verify-otp`, {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          ...
          if (result.status === 200) {
            ...
            handleHttpStatusCode(
              500,
              "Lỗi hệ thống",
              "Đã xảy ra lỗi trong quá trình xác thực mã OTP. Vui lòng thử lại sau."
            );
            return;
          } else {
      ...
      <div className="flex flex-col justify-center items-center h-[80vh] w-[60vh] border border-black bg-white">
        ...
        >
          ...
          <div className="w-full py-6 px-6">
            {isEmptyOTP ? (
              <Button htmlType="submit" disabled size="large" block>
                Xác nhận
              </Button>
            ) : (
              <Button
        ...
        <div className="w-full py-6 px-6 flex flex-col items-center">
          ...
          <p className="text-sky-700 font-bold text-2xl">
            {formatTime(seconds)}
          </p>
        </div>
      </div>
