...
const Signup = () => {
  ...
  const validateForm = () => {
    ...
    if (phone.length < 10 || phone.length > 11) {
      ...
      valid = false;
      notifyErrorWithCustomMessage(
        "Số điện thoại không hợp lệ, vui lòng kiểm tra lại thông tin 🫠",
        messageApi
      );
    }
    return valid;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${apiUrl}v1/auth/register/send-otp`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        if (result.status === 200) {
          ...
          notifySuccessWithCustomMessage(
            "Gửi mã OTP thành công, vui lòng kiểm tra tin nhắn SMS của bạn để xác thực tài khoản 🥳",
            messageApi
          );
          setTimeout(() => {
      ...
      <div className="flex flex-col justify-center items-center h-[80vh] w-[60vh] border border-black bg-white">
        ...
        >
          ...
          <div className="w-full h-fit px-6 items-start">
            ...
            >
              Bằng việc đăng ký tài khoản tại HCMUTE Care, tôi đã đọc và đồng ý
              với các <span className="text-sky-700">điều khoản sử dụng</span>
            </Checkbox>
          </div>
