...
} from "../../../utils/notificationHelper"; // Import hàm xử lý thông báo dựa trên status code
const DoctorManageRecords = () => {
  ...
  const handleSelectPatient = async (pat) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}v1/medical-records/${pat.medicalRecordId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSelectSlot = async (slotId) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}v1/schedule-slots/${slotId}/tickets?status=CONFIRMED`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleOpenRecord = async (patient) => {
    ...
    ); // Lấy medicalRecordId từ localStorage
    ...
    <div className="w-full h-full bg-gray-50">
      ...
      <div className="bg-white p-6 shadow-sm border-b border-gray-200">
        ...
        <p className="text-gray-500 mt-1">
          Xem và quản lý hồ sơ bệnh án của bệnh nhân
        </p>
      </div>
                ...
                schedulePatientData.map((pat, index) => (
                  <div
                    ...
                    className={`p-4 border-b border-gray-100 hover:bg-blue-50 cursor-pointer transition duration-150 ${
                      selectedPatient === pat ? "bg-blue-50" : ""
                    }`}
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex gap-4 mt-2 text-sm text-gray-600 w-full">
                        ...
                        <span>
                          ...
                          {selectedPatient.patient?.career ||
                            "Không có dữ liệu"}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="flex flex-col">
                      ...
                      <span className="font-medium">
                        {selectedPatient.patient?.address || "Không có dữ liệu"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-gray-500">Email</span>
                      <span className="font-medium">
                        {selectedPatient.patient?.email || "Không có dữ liệu"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-gray-500">Số điện thoại</span>
                      <span className="font-medium">
                        {selectedPatient.patient?.phone || "Không có dữ liệu"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-gray-500">Quốc tịch</span>
                      <span className="font-medium">
                        {selectedPatient.patient?.nation || "Không có dữ liệu"}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="h-full flex items-center justify-center p-8 text-center text-gray-500">
                <div>
                  ...
                  <p className="max-w-md mx-auto">
                    Vui lòng chọn một bệnh nhân từ danh sách bên trái để xem
