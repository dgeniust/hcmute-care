  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}v1/customers/${customerId}/payments?paymentStatus=CANCELLED&page=1&size=10&sort=paymentDate&direction=desc`, 
        {
          method: 'GET',
          headers: {
              ...
              description={
                <div className="text-center">
                  ...
                  <p className="text-gray-500 mt-2">
                    Các giao dịch của bạn sẽ được hiển thị ở đây sau khi thanh toán
                  </p>
                </div>
              } 
            />
      ...
      >
        <div className="grid grid-cols-1 gap-4">
          {transactions.map(transaction => (
            <div 
              key={transaction.id}
              className={`p-4 rounded-lg cursor-pointer transition-all ${getStatusColor(transaction.paymentStatus)}`}
              ...
              <div className="flex justify-between items-center">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600 mt-1">
                    <span className="font-medium">Mã GD:</span> {transaction.transactionId || 'N/A'}
                  </div>
                  {transaction.paymentMethod && (
                    <div className="text-sm text-gray-600 mt-1">
                      <span className="font-medium">Phương thức:</span> {getPaymentMethod(transaction.paymentMethod)}
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-6">
            ...
            {selectedTransaction && (
              <Card className="bg-blue-50 border border-blue-200">
                <div className="flex justify-between items-center">
                  <Title level={4} className="m-0 text-blue-700">Chi tiết thanh toán</Title>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="font-bold text-xl text-blue-700">{formatAmount(selectedTransaction.amount)}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium">{selectedTransaction.transactionId || 'N/A'}</div>
                  </div>
                  <div>
                    <Text type="secondary">Phương thức:</Text>
                    <div>{getPaymentMethod(selectedTransaction.paymentMethod)}</div>
                  </div>
                </div>
              </Card>
            )}
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-semibold">{appointmentDetails.medicalRecord.patientName}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div>{appointmentDetails.medicalRecord.gender === 'MALE' ? 'Nam' : 'Nữ'}</div>
                </div>
                <div>
            ...
            {appointmentDetails.tickets && appointmentDetails.tickets.length > 0 && (
              <Card 
                title={
                  <div className="flex items-center">
                    ...
                    appointmentDetails.tickets.map((ticket) => (
                        <div className="space-y-4" key={ticket.id}>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="font-semibold">{ticket.ticketCode}</div>
                                </div>
                                <div>
                                <Text type="secondary">Ngày khám:</Text>
                                <div className="font-semibold text-green-700">{ticket.schedule.date}</div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="ml-6">
                                <div className="font-semibold">{ticket.schedule.doctor.fullName}</div>
                                <div className="text-gray-600 text-sm">{ticket.schedule.doctor.qualification}</div>
                                </div>
                            </div>
            ...
            >
              <Timeline
                items={[
                  {
                    color: appointmentDetails.tickets[0].status === 'CONFIRMED' || appointmentDetails.tickets[0].status === 'COMPLETED'? 'blue' :
                    appointmentDetails.tickets[0].status === 'PENDING'? 'green' : 'red',
                    children: (
                      <div>
                        {
                            appointmentDetails.tickets[0].status === 'CONFIRMED' || appointmentDetails.tickets[0].status === 'COMPLETED' ? 
                            <div className="font-semibold">Đặt lịch thành công</div> : 
                            appointmentDetails.tickets[0].status === 'PENDING' ? 
                            <div className="font-semibold">Chờ thanh toán</div> : <div className="font-semibold">Hủy đặt lịch</div>
                        }
                        <div className="text-gray-500 text-sm">{formatDate(appointmentDetails.tickets[0].date)}</div>
                      </div>
                    ),
                  },
                  {
                    color: appointmentDetails.tickets[0].status === 'CONFIRMED' || appointmentDetails.tickets[0].status === 'COMPLETED'? 'blue' : 'gray',
                    children: (
                      <div>
                        {
                            appointmentDetails.tickets[0].status === 'CONFIRMED' || appointmentDetails.tickets[0].status === 'COMPLETED'?
                            <div className="font-semibold">Thanh toán thành công</div> : <div className="font-semibold">Chưa thanh toán</div>
                        }
                        <div className="text-gray-500 text-sm">{selectedTransaction && formatDate(selectedTransaction.paymentDate)}</div>
                      </div>
                    ),
                  },
                  {
                    color: appointmentDetails.tickets[0].status === 'COMPLETED' ? 'blue' :
                        appointmentDetails.tickets[0].status === 'CONFIRMED' ? 'green':'gray',
                    children: (
                      <div>
                        <div className="font-semibold">
                          {appointmentDetails.tickets[0].status === 'COMPLETED' 
                            ? 'Đã hoàn thành khám' 
                            :appointmentDetails.tickets[0].status === 'CONFIRMED' ? 'Chờ khám' : 'Hủy khám'}
                        </div>
                        {appointmentDetails.tickets[0].status === 'COMPLETED' && (
                          <div className="text-gray-500 text-sm">{formatDate(appointmentDetails.tickets[0].schedule.date)}</div>
                        )}
                      </div>
                    ),
                  },
