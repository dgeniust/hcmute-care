    ...
    try {
      ...
      if (records.length > 0) {
        ...
        if (filteredRecords.length > 0) {
          ...
          console.log(
            "Updated medicalRecords state find record:",
            filteredRecords
          );
          notifySuccessWithCustomMessage(
    ...
    <div className="w-full h-fit min-h-[460px] p-8 flex flex-col gap-4">
      <div className="flex flex-col">
        ...
        <div className="w-full h-fit flex flex-row gap-4 items-center mt-4">
          <Input
            style={{ fontWeight: "bold", width: "100%", height: "50px" }}
            placeholder="N18-000XXXX"
        ...
        <div className="space-y-1 mt-1">
          {medicalRecords && medicalRecords.length > 0 ? (
            ...
            <div className="flex flex-row gap-4 items-center">
              <InfoCircleTwoTone />
              <p className="text-[#273c75] items-center text-center font-bold text-sm cursor-pointer hover:text-blue-500">
      ...
      <div className="text-black flex flex-col gap-4 text-base">
        <div className="flex flex-row gap-4 items-center">
          <InfoCircleTwoTone />
          <p>
            Số hồ sơ được in trên toa thuốc, phiếu chỉ định hoặc phiếu trả kết
            quả cận lâm sàng{" "}
          </p>
        </div>
        <p
          className="text-[#273c75] items-center text-center font-bold text-sm cursor-pointer hover:text-blue-500"
          onClick={showImgSuggest}
        >
          Xem gợi ý <ArrowRightOutlined />
        </p>
      </div>
      {visible ? (
        <div className="img-suggest w-full h-[360px] border border-black"></div>
      ) : null}
      {contextHolder}
    </div>
  );
