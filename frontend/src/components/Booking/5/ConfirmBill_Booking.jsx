...
import { notifyErrorWithCustomMessage, notifySuccessWithCustomMessage } from '../../../utils/notificationHelper';
...
const ConfirmBill_Booking = () => {
    ...
    useEffect(() => {
        try {
            ...
            if (storedData) {
                ...
                notifySuccessWithCustomMessage('Vui lòng kiểm tra thông tin đặt lịch khám của bạn tại đây!', messageApi);
            }
        } catch (error) {
    ...
    const patientInfo = paymentData?.appointment?.medicalRecord || {};
    const ticket = paymentData?.appointment?.tickets?.[0] || {};
        ...
        <div className="w-fit h-fit min-h-[460px] border border-sky-600 px-8 py-4 space-y-8 rounded-lg justify-center items-center flex flex-col m-auto">
            <div className="flex flex-col text-black justify-center items-center space-y-4 text-center">
                ...
                <div className="flex flex-col">
                    ...
                    <span>(Mã giao dịch: {paymentData?.transactionId || 'Đang tải...'})</span>
                </div>
            </div>
            <div className="flex flex-col space-y-4 items-center justify-center text-center">
                <div className="flex flex-col space-y-1 text-black items-center justify-center text-center">
                    ...
                    <p>{ticket.scheduleSlot?.roomName || 'Phòng 33 - Khám Nội Lầu 1 Khu A'}</p>
                    <div className="w-[80px] h-[80px] border border-[#2ecc71] rounded-full flex flex-col justify-center items-center p-4 text-[#2ecc71]">
                        <p className="text-4xl font-bold text-[#009432]">{ticket.waitingNumber || '--'}</p>
                        <span>STT</span>
                    </div>
                </div>
                <div className="flex flex-row items-center space-x-20 justify-center w-full text-center text-black">
                    <div className="space-y-4">
                        <div className="flex flex-row space-x-4">
                            <h1>Họ tên:</h1>
                            <p className="font-bold">{patientInfo.patientName || 'Đang tải...'}</p>
                        </div>
                        <div className="flex flex-row space-x-4">
                            <h1>Ngày sinh:</h1>
                        ...
                        <div className="flex flex-row space-x-4">
                            ...
                            <p className="font-bold">
                                {patientInfo.gender === 'MALE' ? 'Nam' : patientInfo.gender === 'FEMALE' ? 'Nữ' : '--'}
                            </p>
                        </div>
                    </div>
                    <div className="space-y-4">
                        <div className="flex flex-row space-x-4">
                            ...
                            <p className="font-bold">
                                {ticket.scheduleSlot?.timeSlot?.startTime && ticket.scheduleSlot?.timeSlot?.endTime
                                    ? `${ticket.scheduleSlot.timeSlot.startTime} - ${ticket.scheduleSlot.timeSlot.endTime}`
                                    : '-:00 - -:00'}
                            </p>
                        </div>
                        <div className="flex flex-row space-x-4 items-center">
                            ...
                            <p className="font-bold text-[#009432] text-lg">
                                {paymentData?.amount ? `${paymentData.amount.toLocaleString()}đ` : '---.000đ'}
                            </p>
                        </div>
                        <div className="flex flex-row space-x-4">
                            <h1>Đối tượng:</h1>
            ...
            <div className="text-black space-y-4 flex justify-center flex-col items-center text-center">
                ...
                <div>
                    <p>
                        Số hồ sơ (Mã bệnh nhân): <span className="font-bold">{patientInfo.barcode || '--'}</span>
                    </p>
