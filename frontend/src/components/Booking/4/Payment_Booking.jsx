...
const MOMO = () => (
  ...
  >
    ...
    >
      ...
      ></path>
      <path
        d="m13.4288,9.4648c0-1.9849,1.8504-3.964,3.9644-3.964,2.1186,0,3.9644,1.9783,3.9644,3.964v11.8915"
        fill="none"
        stroke="#D82D8B"
        strokeLinecap="round"
        strokeLinejoin="round"
    ...
    try {
      ...
      if (data) {
        if (data.status === 200 && data.data) {
          setTimeout(() => {
            notifySuccessWithCustomMessage(
              "Đặt khám thành công, đang chuyển hướng tới trang thanh toán",
              messageApi
            );
          }, 500);
  ...
  return (
    <div className="w-full h-fit min-h-[460px] border border-red-600 p-8 bg-gray-100 space-y-4">
      <div className="rounded-lg space-y-2">
        <div className="text-black w-full h-fit p-4 text-base">
          ...
          <p className="text-[#273c75] font-bold">
            Sau khi thanh toán thành công, bạn vui lòng đợi nhận{" "}
            <span className="italic">PHIẾU KHÁM BỆNH</span>, không đóng ứng
            dụng.
          </p>
        </div>
        <div className="flex flex-col" ref={ref1}>
          <div className="flex flex-row space-x-4 bg-white p-4">
            ...
            <h1 className="text-[#273c75] font-bold text-base">
      ...
      <div>
        <Button
          type="primary"
          className="w-full h-12 bg-[#273c75] hover:bg-[#273c75] text-white font-bold"
