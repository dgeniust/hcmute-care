  ...
  useEffect(() => {
    const handleDataSpecialty = async () => {
      try {
        ...
        notifyErrorWithCustomMessage(
          "Lỗi kết nối khi cập nhật chuyên khoa",
          messageApi
        );
      ...
      <div className="w-full max-h-[460px] h-[460px] flex flex-col border border-black rounded-xl overflow-y-auto space-y-2">
        {specialtyData && specialtyData.length > 0 ? (
          specialtyData.map((item) => (
            ...
            >
              <div
                className="w-full h-fit flex flex-row items-center justify-between mt-4"
              ...
              >
                <div className="flex flex-row items-center space-x-2 justify-start">
                  ...
                  <p className="text-[#273c75] text-sm font-bold">
                    {item.name}
                  </p>
                </div>
