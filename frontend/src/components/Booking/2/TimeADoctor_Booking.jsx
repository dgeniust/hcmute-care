...
const TimeADoctor_Booking = ({handleSlotClick}) => {
    ...
    useEffect(() => {
        const handleDataTimeADoctor = async () => {
            try {
                const response = await fetch(`http://localhost:8080/api/v1/schedules/available?medicalSpecialtyId=${specialtyId}&date=${dateBooking}`, {
                    method: 'GET',
                    headers: {
                ...
                if(!response.ok) {
                    ...
                    handleHttpStatusCode(response.status, '',`Lấy lịch khám thất bại: ${errorText || response.statusText}`, messageApi);
                    return;
                }
                ...
                notifyErrorWithCustomMessage('Có lỗi xảy ra trong quá trình lấy lịch khám. Vui lòng thử lại sau.', messageApi);
            }
        }
