...
const doctorFullname =
  localStorage.getItem("userFullName") || "Bs. ........................";
// Tiêu đề quốc gia và bệnh viện tái sử dụng
const HeaderSection = ({
  hospital = "BỆNH VIỆN ĐẠI HỌC Y DƯỢC THÀNH PHỐ HỒ CHÍ MINH",
  documentNumber = "",
}) => {
    ...
    <View style={styles.header}>
      ...
      <View style={styles.hospitalInfo}>
        ...
        <Text style={styles.documentNumber}>
          Số: {documentNumber || `....../${today.getFullYear()}`}
        </Text>
      </View>
    </View>
  );
    ...
    <View style={styles.row}>
      ...
      <Text style={styles.value}>
        {patientInfo?.patient?.dob
          ? formatDate(patientInfo.patient.dob)
          : "N/A"}
      </Text>
    </View>
    <View style={styles.row}>
      ...
      <Text style={styles.value}>
        {patientInfo?.patient.gender === "MALE" ? "Nam" : "Nữ" || "N/A"}
      </Text>
    </View>
    <View style={styles.row}>
...
const FooterSection = () => {
  ...
  return (
    <View style={styles.footer}>
      <Text>
        Báo cáo được tạo vào {today.toLocaleDateString("vi-VN")} -{" "}
        {today.toLocaleTimeString("vi-VN")}
      </Text>
      <Text>
        Kết quả này chỉ có giá trị tại bệnh viện. Mọi sao chép đều không có giá
        trị pháp lý.
      </Text>
    </View>
  );
...
const LabTestsPDF = ({ labTests, patientInfo }) => (
  <Document>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.row}>
        <Text style={styles.value}>
          Các kết quả xét nghiệm trên chỉ có giá trị tham khảo và cần được đánh
          giá bởi bác sĩ chuyên khoa.
        </Text>
      </View>
            ...
            <View style={styles.row}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      test.status === "COMPLETED" ? "#52c41a" : "#1890ff",
                  },
                ]}
              />
...
const PDFExportButtons = ({ labTests, imagingTests, functionalTests }) => {
  ...
  useEffect(() => {
    const fetchDoctorInfo = async () => {
      const patientEncounterInfo = JSON.parse(
        localStorage.getItem("patientEncounterInfo")
      );
      try {
        const response = await fetch(
          `${apiUrl}v1/medical-records/${patientEncounterInfo.id}`,
          {
            method: "GET",
            headers: {
  ...
  const safeFunctionalTests = Array.isArray(functionalTests)
    ? functionalTests
    : [];
  ...
  return (
    <Space>
      <PDFDownloadLink
        document={
          <LabTestsPDF labTests={safeLabTests} patientInfo={patientInfo} />
        }
        fileName={`${patientInfo.id}-bao-cao-xet-nghiem.pdf`}
        style={{ textDecoration: "none" }}
      >
        {({ blob, url, loading, error }) => (
          <Button icon={<FilePdfOutlined />} loading={loading} disabled={error}>
      ...
      <PDFDownloadLink
        document={
          <FunctionalTestsPDF
            functionalTests={safeFunctionalTests}
            patientInfo={patientInfo}
          />
        }
