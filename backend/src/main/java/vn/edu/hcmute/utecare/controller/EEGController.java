...
public class EEGController {
    ...
    @GetMapping("/by-date")
    @Operation(summary = "Lấy danh sách eeg theo ngày", description = "Lấy danh sách eeg của ngày được chỉ định với trạng thái PENDING.")
    public ResponseData<List<EEGResponse>> getAllLabTestByDateAndStatus(
            @RequestParam @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE) LocalDate date,
            @RequestParam("status") String status
    ) {
        log.info("Yêu cầu lấy danh sách eeg theo ngày: {}", date);
        List<EEGResponse> responses = eegService.getAllLabTestByDateAndStatus(date, status);
        return ResponseData.<List<EEGResponse>>builder()
                .status(HttpStatus.OK.value())
                .message("Danh sách eeg theo ngày được trả về thành công")
                .data(responses)
                .build();
    }
}
